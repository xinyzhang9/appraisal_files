...
//@TODO: Refactor to using controller.  Currently this does not allow 2 progress-spinner to be active on the page.
var directive;
(function (directive) {
    'use strict';
    var ProgressSpinner = (function () {
        function ProgressSpinner(templateRoot) {
            var _this = this;
        ...
        Object.defineProperty(ProgressSpinner.prototype, "templateUrl", {
            get: function () {
                return this.templateRoot + 'consumersite/directives/progressspinner/progress-spinner.html';
            },
            enumerable: true,
            configurable: true
        });
        ProgressSpinner.createNew = function (args) {
            return new ProgressSpinner(args[0]);
        };
        ProgressSpinner.prototype.setUpSVG = function ($scope, $element) {
            // set up margins
            //aalt 2016-2-29: Use the angular element instead of a class selector 
            var el = d3.select($element.find('.spinner')[0]), elWidth = parseInt($scope.height, 10), elHeight = parseInt($scope.width, 10), margin = { top: 5, right: 5, bottom: 5, left: 5 };
        ...
        ProgressSpinner.prototype.addText = function (textColor) {
            ...
            }).attr('class', 'text-percentage').attr('text-anchor', 'middle').attr('y', '5px').attr("font-size", function () {
        ...
        ProgressSpinner.$inject = ['templateRoot'];
