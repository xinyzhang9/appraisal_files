...
(function (cls) {
    ...
    var BorrowerViewModel = (function (_super) {
        ...
        function BorrowerViewModel(ti, borrower) {
            ...
            this.initialize = function (borrower) {
                ...
                if (!_this.declarationsInfo)
                    _this.declarationsInfo = new cls.DeclarationInfoViewModel();
                else {
                    _this.declarationsInfo = new cls.DeclarationInfoViewModel(_this.declarationsInfo);
                }
            ...
            */
            ...
            /**
            * @desc: Fucntion is used to get annual income amount and sources for Borrower and CoBorrower.
            *        Amount and sources are calculated for Current Employment (all categories) and for specfic categories of other incomes:
            *        - AlimonyChildSupport
            ...
            *        - TipIncome
            */
            this.getBorrowerAndCoBorrowerAnnualSourcesOfIncome = function (lookups) {
                var sources = '';
                lib.forEach(_this.getCurrentEmploymentInfo().getIncomeInformation(), function (item) {
                    if (!!item.calculatedMonthlyAmount) {
                        sources += item.incomeType + ', ';
                    }
                });
                lib.forEach(_this.getOtherIncomes(), function (item) {
                    if (!!item.calculatedMonthlyAmount) {
                        if (item.incomeTypeId == 8 /* AlimonyChildSupport */ || item.incomeTypeId == 17 /* RetirementPensionIncome */ || item.incomeTypeId == 14 /* SocialSecurity */ || item.incomeTypeId == 15 /* DisabilityIncome */ || item.incomeTypeId == 7 /* TrustIncome */) {
                            sources += lookups.getText(lookups.incomeTypesOther, String(item.incomeTypeId)) + ', ';
                        }
                    }
                });
                return sources.substring(0, sources.length - 2);
            };
            /*
            * Get all comma separated income sources for borrower
            */
            this.getBorrowerSourcesOfIncome = function () {
                var sources = '';
                lib.forEach(_this.getEmploymentInfoes(), function (e) { return sources = sources + (e.name != null ? e.name : '') + ', '; });
                var otherIncomes = _this.getOtherIncomes();
                lib.forEach(otherIncomes, function (o) { return sources = sources + lib.getLookupStringByEnum(_this.incomeTypes, o.incomeTypeId) + ', '; });
                return sources.substring(0, sources.length - 2);
            };
            this.getBorrowerTotalIncomeAmount = function () {
                var totalIncomeAmount = 0;
                var employmentInfoes = _this.getEmploymentInfoes();
                lib.forEach(employmentInfoes, function (e) { return totalIncomeAmount += (e.totalMonthlyAmount * 12); });
                var otherIncomes = _this.getOtherIncomes();
                lib.forEach(otherIncomes, function (o) { return totalIncomeAmount += (o.calculatedMonthlyAmount * 12); });
            ...
            */
            ...
            this.prepareForSubmit = function () {
                lib.tryAction(function () {
                    _this.getCurrentEmploymentInfo().prepareForSubmit();
                    lib.forEach(_this.getAdditionalEmploymentInfo(), function (additionalEmploymentInfo) {
                        additionalEmploymentInfo.prepareForSubmit();
                    });
                });
            };
        ...
        Object.defineProperty(BorrowerViewModel.prototype, "incomeTypes", {
            get: function () {
                var mortgageType = cls.LoanViewModel.getMortgageType();
                var hideAdditionalIncomes = cls.LoanViewModel.getMortgageType() == 4 /* VA */ || cls.LoanViewModel.getMortgageType() == 1 /* FHA */ || cls.LoanViewModel.getMortgageType() == 5 /* USDA */;
                return _.filter(cls.LoanViewModel.getLookups().incomeTypesOther, function (incomeType) {
                    return !(incomeType.value == 16 /* SelfEmployedIncome */ || (hideAdditionalIncomes && (incomeType.value == 30 /* CapitalGains */ || incomeType.value == 31 /* EmploymentRelatedAssets */ || incomeType.value == 32 /* ForeignIncome */ || incomeType.value == 35 /* RoyaltyPayment */ || incomeType.value == 36 /* SeasonalIncome */ || incomeType.value == 37 /* TemporaryLeave */)));
                });
            },
            enumerable: true,
            configurable: true
