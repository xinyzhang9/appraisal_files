(function () {
    ...
    angular.module('iMP.Filters')
        ...
        .filter('impDateFormat', impDateFormat)
        .filter('showInitials', showInitials)
        .filter('impIncomePaymentPeriod', impIncomePaymentPeriod)
        .filter('impEmploymentPaymentPeriod', impEmploymentPaymentPeriod)
...
/**
    ...
    */
    ...
    function impDateFormat($filter) {
        return function (input, passedFormat) {
            var retVal = '';
            var format = !!passedFormat ? passedFormat : 'MM/DD/YYYY'
            if ($.trim(input).length > 0) {
                retVal = moment(input).format(format);
            }
    ...
    function impIncomePaymentPeriod() {
        return function (input) {
            var result = "";
            var paymentPeriod = srv.PeriodTypeEnum[srv.PeriodTypeEnum[input]];
            ...
            switch (paymentPeriod) {
                case srv.PeriodTypeEnum.Monthly:
                    result = "mo";
                    break;
                case srv.PeriodTypeEnum.Annually:
                    result = "yr";
                    break;
                default:
                    result = "";
    ...
    function impEmploymentPaymentPeriod() {
        return function (input) {
            var result = "";
            var paymentPeriod = srv.preferredPaymentPeriodsTypeEnum[srv.preferredPaymentPeriodsTypeEnum[input]];
