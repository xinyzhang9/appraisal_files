...
(function (_documents) {
    var DocumentsService = (function () {
        function DocumentsService($http, apiRoot, docVaultSvc, $log, $resource) {
            var _this = this;
            this.$http = $http;
            ...
            //if borrowers needs list gets refactored
            //move sorting logic to backend
            this.sortAlphabetically = function (documents, applicationData) {
                var self = _this;
                var toSortDocuments = self.filterDocumentList(documents, applicationData);
                if (toSortDocuments && toSortDocuments.length > 1) {
                    //helping hashtable of documents to be sorted and their ids
                    var toSortDocumentsIds = {};
                    toSortDocuments.forEach(function (doc) {
                        toSortDocumentsIds[doc.documentId] = doc;
                    });
                    //extract documents that are not being sorted (no docvaulttype mapping)
                    var notForSortDocuments = documents.filter(function (doc) {
                        return !(doc.documentId in toSortDocumentsIds);
                    });
                    //sort documents based on their DocVaultType full description
                    toSortDocuments.sort(function (x, y) {
                    ...
                    //return all documents back, unsorted ones are filtered inside angular ngRepeat
                    return toSortDocuments.concat(notForSortDocuments);
                }
                else {
                    return documents;
                }
            };
            this.getDocumentList = function (documentListRequestEnvelope) {
                var path = _this.apiRoot + 'DocumentRepository/GetDocumentList';
                return _this.$resource(path).save(documentListRequestEnvelope);
            };
            this.doesWetSignaturePackageExist = function (doesWetSignaturePackageExistRequestEnvelope) {
                var path = _this.apiRoot + 'DocumentRepository/DoesWetSignaturePackageExist';
                return _this.$resource(path).save(doesWetSignaturePackageExistRequestEnvelope);
            };
            this.createAuditLogItem = function (appraisalAuditLogRequest) {
                var path = _this.apiRoot + 'DocumentRepository/CreateAuditLogItem';
                return _this.$resource(path).save(appraisalAuditLogRequest);
            };
            this.getMyDocuments = function (myDocumentsRequest) {
                var path = _this.apiRoot + 'DocumentRepository/GetMyDocuments';
                return _this.$resource(path).save(myDocumentsRequest);
            };
            this.docVaultTypes = [];
        }
        ...
        DocumentsService.$inject = ['$http', 'apiRoot', 'docVaultSvc', '$log', '$resource'];
