...
(function (cls) {
    var LoanViewModel = (function (_super) {
        ...
        function LoanViewModel(loan, $filter, isWholeSale) {
            ...
            this.isMortgageTypeVa = function () {
                return _this.financialInfo && _this.financialInfo.mortgageType == 4 /* VA */;
            };
            this.fhaTabEnabled = function () {
                return _this.financialInfo && _this.financialInfo.mortgageType == 1 /* FHA */;
            };
            ...
            */
            this.remainingOwnershipCalculation = function (excludeActiveLoanApplication) {
                ...
                var isNumberOfUnitsGreaterThanOneForPrimaryResidence = _this.subjectProperty.isNumberOfUnitsGreaterThanOneForPrimaryResidence();
                var investmentPropertyLoanApplications = lib.filter(loanApplications, function (la) { return la.OccupancyType == 2 /* InvestmentProperty */ || isNumberOfUnitsGreaterThanOneForPrimaryResidence; });
            ...
            this.isWheelRunning = function () {
                return _this.isCalculatorRunning || _this.active.isCreditRunning;
            };
            ...
            this.allPiecesAcquiredForPrimaryLoanApplication = function () {
                return _this.primary.areSixPiecesOfLoanApplicationCompleted() && _this.isPropertyPieceAcquired();
            };
            /**
            ...
            */
            this.isLoanApplicationCompleted = function (loanApplication) {
                return loanApplication.isLoanApplicationCompleted(_this.getSubjectProperty(), _this.loanPurposeType, _this.loanAmount, !_this.active.getCoBorrower().getCurrentAddress().isSameAsPrimaryBorrowerCurrentAddress, _this.financialInfo.downPaymentTypeCode, _this.areSixPiecesAcquiredForCurrentLoanApplication(), _this.purposeOfRefinance);
            };
            ...
            this.areSixPiecesAcquiredForAllLoanApplications = function () {
                var rslt = false;
                try {
                    rslt = _this.areSixPiecesAcquiredForAllLoanApplicationsImpl();
                }
                catch (e) {
                    console.warn(e);
                    rslt = false;
            ...
            this.areSixPiecesAcquiredForAllLoanApplicationsImpl = function () {
                _this.sixPiecesAcquiredForAllLoanApplications = true;
                var milestoneStatus = new cls.MilestoneStatus();
                _this.primaryAppSixPieces = _this.allPiecesAcquiredForPrimaryLoanApplication();
                ...
                _this.currentMileStoneConsumerSite = _this.currentMilestone;
                if (!_this.isMilestoneStatusManual && _this.currentMilestone != 3 /* processing */ && _this.currentMilestone != 9 /* cancelled */ && _this.currentMilestone != 14 /* adverse */) {
                    _this.currentMilestone = milestoneStatus.setStatus(_this.isWholeSale, allLoanAppsCompleted, _this.sixPiecesAcquiredForAllLoanApplications, _this.homeBuyingType, _this.isCreditRetrievedForAllLoanApplications(), _this.loanPurposeType);
                }
                return _this.sixPiecesAcquiredForAllLoanApplications;
            };
            /**
            * @desc Returns true if all loan applications have the credit report retrieved.
            */
            this.isCreditRetrievedForAllLoanApplications = function () {
                var retVal = true;
                lib.forEach(_this.getLoanApplications(), function (loanApp) {
                    if (loanApp.credit.creditStatus != 2 /* retrieved */) {
                        retVal = false;
                    }
                });
                return retVal;
            ...
            this.checkDisclosureStatusRules = function () {
                if (!_this.applicationDate || moment('0001-01-01').diff(moment(_this.applicationDate)) == 0) {
            ...
            this.updateDisclosureErrors = function (disclosureErrors) {
                var loanApps = _this.getLoanApplications();
                for (var i = 0; i < loanApps.length; i++) {
                    loanApps[i].disclosureErrors = lib.filter(disclosureErrors, function (x) { return x.loanId == loanApps[i].loanApplicationId; });
                }
            };
            this.updateComplianceCheckErrors = function (complianceCheckErrors) {
                var loanApps = _this.getLoanApplications();
                for (var i = 0; i < loanApps.length; i++) {
                    loanApps[i].complianceCheckErrors = lib.filter(complianceCheckErrors, function (x) { return x.loanId == loanApps[i].loanApplicationId; });
                }
            };
           ...
           *---------------------------------------*/
            ...
            this.formatProgramName = function () {
                var name = '';
                var amortizationType = _this.financialInfo.amortizationType, mortageType = _this.financialInfo.mortgageType, loanAmortizationTerm = _this.financialInfo.term, IsBaloonPayment = _this.financialInfo.isBaloonPayment != null ? _this.financialInfo.isBaloonPayment : false, isHarp = _this.loanIsHarp, loanAmortizationFixedTerm = _this.financialInfo.fixedRateTerm;
            ...
            */
            ...
            /**
            * @desc: Retrieves absolute value of cashFromToBorrower IF the value is less then zero
                     If the value was positive in the first place,
                     return 0 (borrower is not retrieving cash)
            */
            ...
            this.initialize = function (loan) {
                ...
                if (loan) {
                    ...
                    if (loan.financialInfo)
                        LoanViewModel._mortgageType = loan.financialInfo.mortgageType;
                    // assign active first
                    ...
                    //initialize FHA scenario info               
                    _this.fhaScenarioViewModel = new cls.FHAScenarioViewModel(_this.getTransactionInfo(), _this.fhaScenarioViewModel);
                    //initialize VA info
                    _this.vaInformation = new cls.VAInformarionViewModel(_this.getTransactionInfo(), _this.vaInformation);
                    if (loan.householdMembers) {
                        for (var i = 0; i < _this.householdMembers.length; i++) {
                            _this.householdMembers[i] = new cls.HouseholdMemberViewModel(_this.getTransactionInfo(), _this.householdMembers[i]);
                        }
                    }
                    // init household members
                    _this.initHouseholdMembers();
                    ...
                    if (!!_this.householdMemberIncomeAndDeductions && _this.householdMemberIncomeAndDeductions.length == 0) {
                        _this.householdMemberIncomeAndDeductions = _this.initialzieHouseholdIncomesAndDeductions();
                    }
                }
                else {
            ...
            this.initHouseholdMembers = function () {
                if (!_this.householdMembers) {
                    return;
                }
                var householdMemberBorrower = lib.findFirst(_this.householdMembers, function (h) { return h.borrowerId == _this.primary.getBorrower().borrowerId; });
                if (householdMemberBorrower == null) {
                    _this.addBorrowerAsHouseholdMember(_this.primary.getBorrower(), 1 /* Borrower */);
                }
                else {
                    if (householdMemberBorrower.source == '') {
                        householdMemberBorrower.source = _this.primary.getBorrower().getBorrowerSourcesOfIncome();
                    }
                }
                var householdMemberCoBorrower = lib.findFirst(_this.householdMembers, function (h) { return h.borrowerId == _this.primary.getCoBorrower().borrowerId; });
                if (_this.primary.isSpouseOnTheLoan && householdMemberCoBorrower == null) {
                    _this.addBorrowerAsHouseholdMember(_this.primary.getCoBorrower(), 2 /* CoBorrower */);
                }
                else if (_this.primary.isSpouseOnTheLoan && householdMemberCoBorrower != null) {
                    if (householdMemberCoBorrower.source == '') {
                        householdMemberCoBorrower.source = _this.primary.getCoBorrower().getBorrowerSourcesOfIncome();
                    }
                }
                else if (!_this.primary.isSpouseOnTheLoan && householdMemberCoBorrower != null) {
                    _this.householdMembers.splice(_this.householdMembers.indexOf(householdMemberCoBorrower), 1);
                }
            };
            this.addBorrowerAsHouseholdMember = function (borrower, memberType) {
                if (borrower == null)
                    return null;
                var householdMember = new cls.HouseholdMemberViewModel(_this.transactionInfo, null, memberType, false, false, lib.getYearsOutOfDate(borrower.dateOfBirth), borrower.borrowerId, borrower.getFullName(), null, null, _this.loanId, borrower.getBorrowerSourcesOfIncome(), null, null);
                _this.householdMembers.push(householdMember);
            };
            this.initialzieHouseholdIncomesAndDeductions = function () {
                var result = [];
                result.push(new cls.HouseholdMemberIncomeAndDeductionsViewModel(null, 1 /* Borrower */, 1 /* Income */));
                result.push(new cls.HouseholdMemberIncomeAndDeductionsViewModel(null, 2 /* CoBorrower */, 1 /* Income */));
                result.push(new cls.HouseholdMemberIncomeAndDeductionsViewModel(null, 3 /* AdditionalIncomeToPrimaryIncome */, 1 /* Income */));
                result.push(new cls.HouseholdMemberIncomeAndDeductionsViewModel(null, 7 /* AnnualChildCareExpenses */, 2 /* Deduction */));
                result.push(new cls.HouseholdMemberIncomeAndDeductionsViewModel(null, 9 /* AnnualDisabilityExpenses */, 2 /* Deduction */));
                result.push(new cls.HouseholdMemberIncomeAndDeductionsViewModel(null, 10 /* AnnualMedicalExpenses */, 2 /* Deduction */));
                result.push(new cls.HouseholdMemberIncomeAndDeductionsViewModel(null, 6 /* DependentDeduction */, 2 /* Deduction */));
                result.push(new cls.HouseholdMemberIncomeAndDeductionsViewModel(null, 8 /* ElderlyHousehold */, 2 /* Deduction */));
                result.push(new cls.HouseholdMemberIncomeAndDeductionsViewModel(null, 4 /* IncomeForAdditionalAdultHouseholdMembers */, 1 /* Income */));
                result.push(new cls.HouseholdMemberIncomeAndDeductionsViewModel(null, 5 /* IncomeFromAssets */, 1 /* Income */));
                return result;
            };
            ...
            this.isGovermentEligible = function () {
                if (_this.financialInfo.mortgageType == 0 /* Conventional */ || _this.financialInfo.mortgageType == 2 /* ConventionalJumbo */ || _this.financialInfo.mortgageType == 3 /* ConventionalSuperJumbo */ || (_this.financialInfo.mortgageType == 1 /* FHA */ && _this.govermentEligibility == 1 /* FHAEligible */) || (_this.financialInfo.mortgageType == 4 /* VA */ && _this.govermentEligibility == 3 /* VAEligible */) || (_this.financialInfo.mortgageType == 5 /* USDA */ && _this.govermentEligibility == 5 /* USDAEligible */)) {
                    return true;
                }
                return false;
            };
            this.isIrrrlEligible = function () {
                if ((_this.incomeVerificationType == 1 /* VAStreamlineCredit */ || _this.incomeVerificationType == 5 /* VAStreamlineNonCredit */) && !(_this.vaInformation.vaCalculatorResults.isEligible && _this.vaInformation.qmCertification.safeHarborReqsFulfilled)) {
                    return false;
                }
                return true;
            };
            this.isAnyNTBActivationsChecked = function (applicationData) {
                var ntbActivations = _this.ntbBenefitActivations;
                for (var i = 0; i < ntbActivations.length; i++) {
                    if (ntbActivations[i].active && lib.findFirst(applicationData.ntbBenefitConfigurations, function (x) { return x.ntbBenefitEnumId == ntbActivations[i].ntbBenefitEnumId && x.state.indexOf(_this.getSubjectProperty().stateName) != -1; }) != null) {
                        return true;
                    }
                }
                return false;
            };
            this.isNtbCompleted = function (applicationData) {
                return !(!_this.FirstLienReoItem || !_this.FirstLienReoItem.loanOriginationDate || !_this.FirstLienReoItem.unpaidBalance || !_this.FirstLienReoItem.reoInfo.loanTerm || !_this.FirstLienReoItem.reoInfo.amortizationType || !_this.FirstLienReoItem.reoInfo.interestRate || !_this.FirstLienReoItem.reoInfo.fullyIndexedRate || _this.FirstLienReoItem.reoInfo.negativeAmortizationFeature == null || _this.negativeAmortization == null || _this.FirstLienReoItem.reoInfo.prePaymentPenalty == null || _this.prePaymentPenalty == null || (!_this.FirstLienReoItem.reoInfo.prePaymentPercentage && _this.FirstLienReoItem.reoInfo.prePaymentPenalty) || (!_this.FirstLienReoItem.reoInfo.prePaymentAmount && _this.FirstLienReoItem.reoInfo.prePaymentPenalty) || (!_this.prePaymentAmount && _this.prePaymentPenalty) || (_this.JuniorLienReoItem && (!_this.JuniorLienReoItem.reoInfo.interestRate || !_this.JuniorLienReoItem.unpaidBalance)) || !_this.FirstLienReoItem.reoInfo.loanPurpose || !_this.isAnyNTBActivationsChecked(applicationData));
            };
            if (loan) {
        ...
        Object.defineProperty(LoanViewModel.prototype, "SelectedAppraisalOrder", {
            get: function () {
                return this.selectedAppraisalOrder = !!this.selectedAppraisalOrder ? this.selectedAppraisalOrder : lib.findFirst(this.appraisalOrders, function (item) {
                    return item.isItemSelected;
                }).orderAppraisals[0];
            },
            set: function (value) {
                this.selectedAppraisalOrder = value;
            },
            enumerable: true,
            configurable: true
        });
        ...
        Object.defineProperty(LoanViewModel.prototype, "ClosingDate", {
            //Alex K: Task 55374:Closing Date - Defaulting to 01/ 01 / 0001 for consumer generated loans
            //included because we set default closing date on client side. When that logic moves to server side we can delete ClosingDate
            get: function () {
        ...
        Object.defineProperty(LoanViewModel.prototype, "currentMileStoneConsumerSite", {
            get: function () {
                return this._currentMileStoneConsumerSite;
            },
            set: function (currentMileStoneConsumerSite) {
                this._currentMileStoneConsumerSite = currentMileStoneConsumerSite;
            },
            enumerable: true,
            configurable: true
        });
        ...
        LoanViewModel.getMortgageType = function () {
            return LoanViewModel._mortgageType;
        };
        ...
        LoanViewModel.prototype.get = function (item) {
            var ntbBenefitActivation = lib.findFirst(this.ntbBenefitActivations, function (x) { return x.ntbBenefitEnumId === item.ntbBenefitEnumId; });
