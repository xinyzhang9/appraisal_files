...
//impCreditCardTypeCheck
angular.module('iMP.Directives').directive('impCreditCardTypeCheck', impCreditCardTypeCheck);
function impCreditCardTypeCheck() {
    return {
        require: 'ngModel',
        link: function (scope, elem, attr, ngModel) {
            var creditCardType = scope.$eval(attr.impCreditCardTypeCheck);

            scope.$watch(attr.impCreditCardTypeCheck, function () {
                //console.log('credit card type watch called');
                var ccnumber = ngModel.$modelValue;
                var valid = creditCardTypeMatch(ccnumber);
                ngModel.$setValidity('impCreditCardTypeCheck', valid);
            });

            //_creditCardTypeOptions = [
            //    { text: 'Mastercard', value: 0 },
            //    { text: 'Visa', value: 1 },
            //    { text: 'Discover', value: 3 }
            //];

            function creditCardTypeMatch(ccnumber) {
                var creditCardType = scope.$eval(attr.impCreditCardTypeCheck);
                //console.log('creditCardType: ' + creditCardType);
                //console.log('ccnumber: ' + ccnumber);
                if (ccnumber == null || ccnumber == '') {
                    //if the ccnumber is empty leave it valid.
                    return true;
                }
                //"appraisalCardTypes": [
                ...
                var isValid = false;
                //Mastercard
                if (creditCardType == 1 && /^5[1-5]/.test(ccnumber)) {
                    isValid = true;
                }
                //Visa
                else if (creditCardType == 2 && /^4/.test(ccnumber)) {
                    isValid = true;
                }
                //Discover
                else if (creditCardType == 4 && /^(6011)|^(622(1(2[6-9]|[3-9][0-9])|[2-8][0-9]{2}|9([01][0-9]|2[0-5])))|^(64[4-9])|^65/.test(ccnumber)) {
                    isValid = true;
                }
            ...
            ngModel.$parsers.unshift(function (value) {
                var valid = creditCardTypeMatch(value);
                ngModel.$setValidity('impCreditCardTypeCheck', valid);
            ...
            ngModel.$formatters.unshift(function (value) {
                ngModel.$setValidity('impCreditCardTypeCheck', creditCardTypeMatch(value));
                return value;
            });
        }
    };
...
function impCreditCardCheck() {
    return {
        ...
        link: function (scope, element, attributes, ngModel) {
            ...
            //For DOM -> model validation
            ngModel.$parsers.unshift(function (value) {
                var valid = isValidCreditCard(value);
                ngModel.$setValidity('imp-credit-card-check', valid);
                return value;
            });

            //For model -> DOM validation
            ngModel.$formatters.unshift(function (value) {
                ngModel.$setValidity('imp-credit-card-check', isValidCreditCard(value));
                return value;
            });
...
impDirectives.directive('impSaveLoader', function () {
    return {
        restrict: 'E',
        scope: {
            width: '=',
        ...
        template:
            '<img style="margin-right: 5px;margin-left: -10px;" src="../../Content/pinwheel.gif" width="{{width}}" height="{{height}}" />'
    };
});
...
impDirectives.directive('impDdlWithModalPopover', function (modalPopoverFactory) {
    return {
        restrict: 'E',
        bindToController: true,
        controllerAs: 'ctrl',
        scope: {
            ...
            horizontalPopupPositionPerWidth: '=?',
            customCssClass: '@?'
        },
        controller: function () {
            ...
            this.showOptions = function (event) {
                ...
                modalPopoverFactory.openModalPopover(this.templateUrl, ctrl, this.options, event, { className: 'tooltip-arrow-display-non', verticalPopupPositionPerHeight: this.verticalPopupPositionPerHeight, horisontalPopupPositionPerWidth: this.horizontalPopupPositionPerWidth, refExtModelAndCtrl: true });
            }

        },
        templateUrl: 'angular/common/ddlwithmodalpopover.html'
    };
});
...
impDirectives.directive('impDate', function ($filter, dateParser, $timeout) {
    return {
        ...
        link: function (scope, element, attrs, ngModelCtrl) {
            ...
            var allowFutureDates = common.objects.isNullOrUndefined(attrs.allowFutureDates) || attrs.allowFutureDates === 'true';
            ...
            var listener = function () {
                ...
                if (!common.date.isValidDate(value, true, allowFutureDates) || value == "" || value == null || value == undefined)
                    return;
            ...
            function parseDate(viewValue) {
                if (!viewValue) {
                    ...
                    if (common.date.isShortDate(viewValue)){
                        return viewValue;
                    }
                    else if (common.date.isMediumDate(viewValue)) {
                        return moment(viewValue).format("MM/DD/YYYY");
                    } 
                    }
                    return undefined;
                }
            ...
            var onBlur = function () {
                ...
                if (!common.date.isValidDate(element.val(), true, allowFutureDates)) {
                    $timeout(function () {
...
impDirectives.directive('impCurrency', function ($filter, $browser, $timeout) {
    return {
        require: 'ngModel',
        link: function (scope, element, attrs, ngModelCtrl) {
            var listenerOnChange = function () {
                ...
                if (attrs.roundCurrency == "true" && !isNaN(value) && !common.string.isNullOrWhiteSpace(value))
                {
                    scope.$apply(function () {
                        value = Math.round(parseFloat(value)).toFixed(2);
                ...
                if (attrs.ngBlur && attrs.roundCurrency == "true")
                    $timeout(function () {
            ...
            ngModelCtrl.$render = function () {
                ...
                if (attrs.roundCurrency == "true" && !isNaN(viewValue))
                    viewValue = Math.round(parseFloat(viewValue)).toFixed(2);
    ...
    return {
        ...
        link: function (scope, element, attrs, ctrl) {
            ...
            var listenerOnChange = function () {
                ...
                // add zero if input has just one decimal --> related to bug 57567
                else if (scope.roundTwoDecimals) {
...
impDirectives.directive('impPhone', function ($filter, $browser, $compile) {
    return {
        ...
        link: function (scope, element, attrs, ngModelCtrl) {
            ...
            var phoneUI = "<div style='width: 100%; display:flex; justify-content: flex-start;'><div style='width: 70%'>\
                           ...
                           <div style='width: 30%;' ng-hide='disablePhoneTypeOptions'>\
        ...
        link: function (scope, element, attrs, ngModelController) {
            ...
            var onBlur = function () {
                ...
                if (scope.innerCustomFilters) {
                    ...
                    if (!common.date.isValidDate(scope.model, true, allowFutureDates)) {
                        $timeout(function () { scope.model = null; });
                    }
                }
                if (!!scope.reoValidation && common.date.isShortDate(scope.model)) {
                    scope.reoValidation();
                }
...
impDirectives.directive('impDocVault', function () {
    return {
        ...
        scope: {
            ...
            cancel: "&?",
            savingInProgres: "=?",
            documentBorrowerViewableChanged: '&?',
            isWheelOn: '&?'
         },
...
impDirectives.directive('validDecimals', function () {
    return {
        require: '?ngModel',
        link: function (scope, element, attrs, ngModelCtrl) {
            if (!ngModelCtrl) {
...
impDirectives.directive('validateSelectElement', function () {
    return {
        restrict: 'A',
        require: '?ngModel',
        link: function (scope, element, attr, ngModelCtrl) {
            if (ngModelCtrl) {
                ngModelCtrl.$isEmpty = function (value) {
                    ...
                    return value == undefined || value == null || value.length === 0 || value == -1;
                }
            }
        }
    }
