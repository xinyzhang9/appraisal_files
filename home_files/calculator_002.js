    ...
    function commonCalculatorSvc($resource, ApiRoot) {
        ...
        function GetLowestMiddleFicoScore(wLoan, applicationData) {
            ...
            if (midFicoScore == Number.MAX_VALUE && wLoan.ref.otherInterviewData.selectedDecisionScoreRange != -1) {
                ...
                if (!wLoan.ref.financialInfo.ficoScore || wLoan.ref.financialInfo.ficoScore == 0)
                {
                    //var nonTraditionalCreditAvailable = lookupContainsValue(applicationData.lookup.ficoScore, srv.FicoScoreTypeEnum.NonTraditionalCredit);
                    if (wLoan.ref.financialInfo.nonTraditionalCreditType > 0 ||  wLoan.ref.otherInterviewData.selectedDecisionScoreRange == srv.FicoScoreTypeEnum.NonTraditionalCredit)
                    {
                        var nonTraditionalCredit = GetDecisionScoreLookupItem(applicationData.lookup.ficoScore, wLoan.ref.otherInterviewData.selectedDecisionScoreRange);
                        return nonTraditionalCredit[0] ? nonTraditionalCredit[0].stringValue : '';
                    }
                }
        ...
        /*
        *@Description - Get LookupItem from provided lookup and with specific value
        */
        function GetDecisionScoreLookupItem(lookup, compareValue)
        {
            return lib.filter(lookup, function (item) {
        ...
        function GetFicoScore(wLoan, applicationData) {
            ...
            if (wLoan.ref.financialInfo.ficoScore && wLoan.ref.otherInterviewData.selectedDecisionScoreRange != srv.FicoScoreTypeEnum.NonTraditionalCredit)
                return wLoan.ref.financialInfo.ficoScore;
            ...
            if (midFicoScore == -1 && wLoan.ref.otherInterviewData.selectedDecisionScoreRange && wLoan.ref.otherInterviewData.selectedDecisionScoreRange != '-1') {
        ...
        /*
        *@Descirption - Calculate Investor Price
        */
        function CalculateInvestorPrice(llpas, initialAdjustedPoints)
        {
            //To get base price it is required to remove all LLPA values
            var llpaValue = 0;
            var investorPrice = 0;
            lib.filter(llpas, function (llpa) {
                llpaValue = llpaValue - llpa.value;
            });

            investorPrice = 100 - initialAdjustedPoints + llpaValue;
            return investorPrice;
        }
        ...
        var calculatorService =
           {
               ...
               GetDecisionScoreLookupItem: GetDecisionScoreLookupItem,
               CalculateInvestorPrice: CalculateInvestorPrice
           }
