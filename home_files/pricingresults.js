(function () {
    ...
    function pricingResultsSvc($resource, apiRoot, $http, $q, $window) {
        ...
        function GetIntegrationXmls(interviewUserAccountId)
        {
            var questions = $resource(pricingResultsApiPath + "GetIntegrationXmls",
            {
                interviewUserAccountId: '@interviewUserAccountId'
            });

            return questions.get({ interviewUserAccountId: interviewUserAccountId });
        }
        ...
        var ApplyLoan = $resource(loanPath + ":productId", { productId: "@productId", currentUserId: "@currentUserId", repricing: "@repricing", taxQuestionsEnabled: "@taxQuestionsEnabled", isFirstTimeHomebuyer: "@isFirstTimeHomebuyer" }, {
                Create: {
                method: "PUT",
         ...
         function GetTaxQuestions(taxQuestions, currentUserId) {
            var questions = $resource(pricingResultsApiPath + "GetTaxQuestionaries", {
                taxQuestions: '@taxQuestions', currentUserId: '@userAccountId'});

            return questions.save({ currentUserId: currentUserId}, taxQuestions);
         }
        ...
        var pricingResultsService =
           {
                ...
                GetTaxQuestions: GetTaxQuestions,
                GetIntegrationXmls: GetIntegrationXmls
        }
