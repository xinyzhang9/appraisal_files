...
(function () {
    ...
    function docVaultController($log, $scope, $state, $modal, $controller, docVaultSvc, encompassSvc, NavigationSvc, enums, modalPopoverFactory, wrappedLoan, applicationData, simpleModalWindowFactory, $timeout, DocVaultService, $q, limitToFilter, commonModalWindowFactory, modalWindowType) {
        var vm = this;
        ...
        vm.documentBorrowerViewableChanged = documentBorrowerViewableChanged;
        ...
        vm.isWheelOn = isWheelOn;

        var _searchCounter = 0;
        var _filterTextTimeout;

        if (applicationData.generalSettings)
            vm.enableExportDocumentToEncompass = _.find(applicationData.generalSettings, function (obj) { return obj.settingValue == srv.GeneralSettingsEnum.EnableExportDocumentToEncompass })

        vm.groupedDocuments = docVaultSvc.groupDocuments(vm.wrappedLoan.ref.documents.docVaultDocuments);

        docVaultSvc.checkDocumentStatuses(vm.applicationData, vm.groupedDocuments, vm.wrappedLoan.ref.documents.docVaultDocuments);
        ...
        function isWheelOn() {
            return vm.wrappedLoan.ref.isWheelRunning();
        }
        ...
        function exportDocumentToEncompass(document) {
            ...
            encompassSvc.encompassExportDocument(vm.wrappedLoan.ref.loanId, applicationData.currentUserId,
                ...
                function (result) {
                    docVaultSvc.checkEncompassStatusForDocument(applicationData, document, vm.wrappedLoan.ref.documents.docVaultDocuments);
            ...
            function (error) {
                ...
                docVaultSvc.addDocumentHistory(applicationData, document, "Export Failed");
            });
        }
        ...
        function documentBorrowerViewableChanged(document) {
            docVaultSvc.documentBorrowerViewableChanged(vm.wrappedLoan, document);
        }
        ...
        function showDocumentHistory($event, document) {
            for (var i = 0; i < document.documentHistory.length; i++) {
                document.documentHistory[i].orderByDate = new Date(document.documentHistory[i].modifiedDate).getTime();
            }
            var ctrl = {
        ...
        function deleteDocument(document) {
            for (var i = 0; i < vm.wrappedLoan.ref.documents.docVaultDocuments.length; i++) {
                if (vm.wrappedLoan.ref.documents.docVaultDocuments[i].repositoryId == document.repositoryId) {
                    vm.wrappedLoan.ref.documents.docVaultDocuments[i].removed = true;
                    vm.groupedDocuments = docVaultSvc.refreshGroupedDocuments(vm.wrappedLoan.ref.documents.docVaultDocuments, vm.groupedDocuments); //this triggers ui update inside docvault ng-repeat
                }
            }
        }
